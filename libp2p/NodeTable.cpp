    ...
    if (newNode && !newNode->pending)
    {
        ...
        shared_ptr<NodeEntry> nodeToEvict;
        {
            ...
            if (s.nodes.size() < s_bucketSize)
            {
                ...
                auto it = std::find(s.nodes.begin(), s.nodes.end(), newNode);
                if (it == s.nodes.end())
                {
                    // if it was not there, just add it as a most recently seen node (i.e. to the
                    // end of the list)
                    s.nodes.push_back(newNode);
                    if (m_nodeEventHandler)
